...
type Props = {
  ...
  trail?: ReactNode
  /** Slot inline al lado del título (ej. botón de ayuda). Siempre visible. */
  action?: ReactNode
 ...
 *   </SectionHeader>
 */
export function SectionHeader({ eyebrow, children, trail, action, size = 'md', className }: Props) {
  return (
    <div className={cn('mb-4 flex flex-wrap items-baseline justify-between gap-3', className)}>
      <div className="max-w-2xl">
        ...
        <div className="mt-1 flex items-center gap-2">
          <h2 className={cn('font-display leading-tight tracking-tight', SIZE[size])}>
            {children}
          </h2>
          {action}
        </div>
