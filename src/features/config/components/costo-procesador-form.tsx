...
import { saveCostoProcesadorDigital } from '../actions'

type Props = { initialValue: number }

export function CostoProcesadorForm({ initialValue }: Props) {
  ...
  async function handleSave() {
    ...
    setSaving(true)
    const result = await saveCostoProcesadorDigital(pct)
    setSaving(false)
    if (result.error) {
    ...
    return (
      <div className="space-y-1">
        ...
        <p className="text-xs text-muted-foreground">
          Se aplica en /caja sobre las ventas digitales para mostrar el neto que
          realmente queda disponible. Default Mercado Pago: 6%.
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-2">
      <Label htmlFor="costo-procesador">Costo del procesador (%)</Label>
